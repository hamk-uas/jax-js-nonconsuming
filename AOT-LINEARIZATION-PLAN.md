...
**Branch:** `aot-linearization-plan` **Scope:** Refactor the autodiff + JIT ownership model from
ad-hoc PE intermediate cleanup into explicit, artifact-scoped lifecycle management. **Execution
model:** Single autonomous agent. No human review gates.

---

## Motivation

The current autodiff pipeline in `src/frontend/linearize.ts` produces correct results but suffers
from ownership fragility:

1. **`disposePeIntermediates` (L213–260)** uses heuristic protected-set logic with
   `unwrapToConcreteArray` wrapper chasing — any new wrapper type or transform composition can break
   the identity checks.
2. **ClosedJaxpr const ownership** relies on timing: `getOrMakeConstTracer` calls `.ref`,
   `ClosedJaxpr.dispose()` drops it, but user disposal or nested transforms can interleave
   unpredictably. Three separate patches were needed (anonymous consts, unreachable Const PETracers,
   user-disposed consts with `refCount ≤ 1` protection).
3. **Scan backward pass** (L1870–2050) manually reconstructs primal residuals, tangent bodies, and
   transpose jaxprs per invocation — no caching, fragile slicing.
4. **`transposeJaxprCache` (L2152)** and `jitCompileCache` (jit.ts L475) are global mutable maps
   requiring coordinated cleanup via `_registerJitCacheDisposer`.
...
Each artifact implements `[Symbol.dispose]` and owns exactly the resources it needs. No global
heuristic disposal. No protected-set chasing.
...
**Exit criteria:** Baseline files exist in `tmp/` with full logs and exit codes. Zero unexpected
failures.
...
**What:** Create `src/frontend/artifacts.ts` with empty type definitions and a barrel re-export from
`src/index.ts`. No logic yet — just types that compile.
...
export interface PullbackArtifact extends Disposable {
  ...
  // Note: PullbackArtifact may be called multiple times with different cotangents.
  // The residuals must not be consumed/destroyed if reuse is expected.
...
- (Do NOT export these internal types from `src/index.ts` yet)
...
**What:** Create `test/artifacts.test.ts` with placeholder tests that import the stub types and
verify they exist. Tests will initially be trivial (`expect(aotLinearize).toBeDefined()`).
...
Factor `partialEvalFlat` (linearize.ts L109) into a two-phase pipeline: **phase 1** traces and
produces a forward jaxpr + residual specification, **phase 2** produces a backward jaxpr via
`transposeJaxpr`.

This milestone does NOT change any public API or ownership semantics. It is a pure internal refactor
that makes the existing code more modular.
...
The `residualSpec` is a new type describing which variables are residuals (currently implicit in the
"known" vs "unknown" PETracer partition).
...
**Refactoring strategy:** The extracted function keeps identical logic — just relocated. No
ownership changes yet. `partialEvalFlat` becomes a thin wrapper calling `buildForwardJaxpr`.
...
**What:** Create `buildBackwardJaxpr(forwardJaxpr, undefPrimals)` that wraps the existing
`transposeJaxpr` (L2165) call. This is a thin wrapper now, but will become the `PullbackArtifact`
constructor later.
...
**Exit criteria:** All tests pass. New function exists and is called from at least one site (can be
a test).
...
**What:** Replace the free function `disposePeIntermediates` (L213–260) with a `ResidualCollector`
class that:
...
This class is the precursor to `ResidualPack`. For now it has identical behavior to the free
function, just encapsulated.
...
**Exit criteria:** All tests pass. `disposePeIntermediates` replaced by `ResidualCollector` with
identical behavior.
...
**What:** Rewrite `linearizeFlat` (L315–340) and `linearizeFlatUtil` (L286–313) to use the extracted
functions from M1.1 and M1.2 instead of inlining the logic.
...
After this task, it returns a `ForwardResult` containing the forward jaxpr and a `ResidualCollector`
(from M1.3).
...
Build real implementations of `PrimalArtifact`, `ResidualPack`, `PullbackArtifact` on top of the M1
extractions.
...
class ResidualPackImpl implements ResidualPack {
  ...
  get arrays(): Tracer[] {
    if (this.#disposed) throw new UseAfterFreeError("ResidualPack disposed");
    return this.#arrays;
  }
...
**Key design decision:** `ResidualPack` retains ownership of the residuals until it is explicitly
disposed. This satisfies the invariant that the VJP pullback may be called multiple times with
different cotangents. The consumer (pullback) reads the arrays but does not consume them.
...
- `ResidualPack.arrays returns arrays`
- `ResidualPack.dispose() frees arrays`
- `ResidualPack.arrays after dispose() throws UseAfterFreeError`
...
**What:** Implement `PullbackArtifact` wrapping a `ClosedJaxpr` (the backward jaxpr) and a compiled
`JitProgram`:
...
class PullbackArtifactImpl implements PullbackArtifact {
  ...
  run(residuals: ResidualPack, cotangents: Tracer[]): Tracer[] {
    ...
    const resArrays = residuals.arrays; // reads, does not consume
...
- `PullbackArtifact.run() can be called multiple times` (reusability invariant)
...
**What:** Implement `PrimalArtifact` wrapping the forward jaxpr. Its `run()` method executes the
forward pass and produces a `ResidualPack`:
...
class PrimalArtifactImpl implements PrimalArtifact {
  ...
  run(primals: Tracer[]): { primalsOut: Tracer[]; residuals: ResidualPack } {
    ...
    const allOut = evalJaxpr(this.#forwardJaxpr.jaxpr, [
      ...this.#forwardJaxpr.consts.map((c) => c.ref),
      ...primals,
    ]);
...
Replace the internals of `vjpFlat`, `linearizeFlat`, and `grad` to use artifacts instead of the
ad-hoc intermediate tracking.
...
**CRITICAL INVARIANT:** Keep the pending flush behavior at L2233 unless replaced by an equivalent
proven mechanism. The flush is required to prevent orphaned Slot refs.
...
vjpFlat(f, primalsIn, auxStore?)
  ...
  → flush pending ops                   // L2233  ← RETAINED
...
vjpFlat(f, primalsIn, auxStore?)
  ...
  → flush pending ops                   // ← RETAINED
  → return [primalsOut, fVjp, dispose]
    where fVjp calls pullback.run(residuals, cts)
    where dispose calls primal.dispose() + pullback.dispose() + residuals.dispose()
```
...
**After rewrite:** `linearizeFlat` calls `aotLinearize`, uses the forward jaxpr for `fLin`. The
closure captures residuals for later tangent evaluation.
...
**Exit criteria:** All tests pass including Deno WebGPU. Transform compositions at all depths
produce correct results with zero leaks.

---

### M4 — Scan backward pass artifact (3–5 days)

The scan backward pass (linearize.ts L1870–2050) is the most complex consumer of the transpose
machinery. This milestone wraps it in artifacts.
...
**What:** Currently, the scan backward pass calls
`transposeJaxpr(tangentBody.jaxpr, tangentBodyUndefPrimals)` (L1909) on every backward invocation.
The result is already cached by `transposeJaxprCache` (L2152), but the tangent body tracing and
undefPrimals construction is repeated.
...
Its `.run(cotangents)` method executes the checkpoint-based backward loop. Its `[Symbol.dispose]()`
frees checkpoint carries and xs residuals.
...
**What:** After M3, the free function `disposePeIntermediates` (L213–260) should have no call sites.
Remove it and the `unwrapToConcreteArray` helper (L201–210).
...
After M2–M4, artifact caches should be self-cleaning (artifacts own their jaxprs, disposing the
artifact frees the jaxpr). Audit remaining caches and simplify the `_registerJitCacheDisposer`
...
1. **Slot-count parity:** `slotCount()` before and after any operation matches (enforced by
   `test/setup.ts` global `checkLeaks`).
2. **Transform composition correctness:** All depth-1 through depth-4 compositions in
   `test/transform-compositions.test.ts` pass.
3. **Scan gradient correctness:** `grad(scan)` matches finite differences for cumsum,
   Cholesky-in-body, and pytree carry patterns.
4. **No new public API breaks:** `aotLinearize` is additive. Existing `grad`, `vjp`, `linearize`,
   `jit` APIs unchanged.
5. **Build succeeds:** `pnpm run check` (TypeScript) passes at every commit.
6. **VJP Reusability:** VJP pullback may be called multiple times with different cotangents.
   Residuals must not be destructively consumed.
