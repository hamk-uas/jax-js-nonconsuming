...
| Stencil body synthesis for halo-VJP                         | Forward bodies use Shrink (from `sliceInDim`), not UDS. `analyzeLinearStencil` walks body for {Shrink, Add, Mul(Lit)} targeting single halo input. Gather-based VJP synthesizes reversed-stencil backward body via `makeJaxpr`; non-linear/unrecognized bodies fall back to pad+add. JVP threads `originalJaxpr`+`originalNumConsts` through BlockMap params so transpose analyzes clean forward body, not JVP-doubled body |
