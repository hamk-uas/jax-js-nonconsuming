...
- **Core library** (`@jax-js/jax`, root `src/`): array API, autodiff (`grad`, `jvp`, `vjp`), JIT
  ...
  - Library namespaces in `src/library/`: `numpy.ts`, `lax.ts`, `nn.ts`, `random.ts`,
    `scipy-special.ts`, `numpy-linalg.ts`, `numpy-fft.ts`, `lax-linalg.ts`, `scipy-linalg.ts`,
    `numpy/` (numpy implementation split across multiple files).
...
- Types: `AssociativeScanOptions` (accessible via `lax.AssociativeScanOptions`, not re-exported at
  top level)
