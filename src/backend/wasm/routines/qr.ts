 ...
 * For each column j = 0..k-1:
 *   1. Compute Householder vector v from work[j:m, j]
 *   2. Apply H = I - tau * v * v^T to work[j:m, j:n]
 *   3. Apply H to Q[j:m, 0:k]
 *
 * @param cg - CodeGenerator instance
 * @param hl - WasmHl helper
...
function genQRSized(
  ...
  return cg.function([cg.i32, cg.i32, cg.i32], [], () => {
    ...
    // We need a work buffer for the m×n matrix (will become R).
    // Allocate on the stack after the parameters by using locals for a scratch
    // area. But wasmblr doesn't have stack allocation easily.
    // Instead, store the working matrix in the R output area temporarily,
    // then extract final R (k×n) from the first k rows.
    // work buffer: we'll use a local pointer from a fixed offset.
    // Actually, we need m×n workspace but R is only k×n.
    // We'll use a dedicated scratch area past the outputs.
    // Convention: caller provides a scratch buffer or we use rPtr as partial workspace.
    //
    // Simplification: Use qPtr area (m×k) as Q workspace and rPtr area (k×n) as
    // partial R workspace. We need m×n workspace though, so we'll do the
    // Householder transformations in-place, storing results carefully.
    //
    // Alternative approach: The caller (dispatchQR) will allocate a temporary
    // m×n scratch buffer (workPtr) and pass it. We'll change the signature to:
    // qr(aPtr, qPtr, rPtr, workPtr)

    const workPtr = cg.local.declare(cg.i32); // param 3 below would be work
    // Actually, let's add a 4th parameter for the scratch buffer
    // We need to change the function signature. Let's make it:
    // qr_work(aPtr, qPtr, rPtr, workPtr)
    // But cg.function signature is already set. Let's restructure.

    // ... redesign: use 4 params
    ...
    const dot = cg.local.declare(ty);
    const vNormSq = cg.local.declare(ty);
    const v0 = cg.local.declare(ty);
    const val = cg.local.declare(ty);

    // Copy A to work buffer (rPtr temporarily holds full m×n? No, rPtr is k×n)
    // We need a different approach. Let's use qPtr as m×k workspace for Q,
    // and we need an m×n workspace. Since rPtr is only k×n, we can't use it.
    //
    // Solution: the function takes 4 args: (aPtr, qPtr, rPtr, workPtr)
    // But we already declared params as [i32, i32, i32].
