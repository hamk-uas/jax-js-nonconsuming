  ...
  test("bitwise operations", async () => {
    ...
    try {
      ...
      let exe = await backend.prepareKernel(
        Kernel.single(2, 4, AluExp.bitCombine(arg1, arg2, "and")),
      );
      ...
      exe = await backend.prepareKernel(
        Kernel.single(2, 4, AluExp.bitCombine(arg1, arg2, "or")),
      );
      backend.dispatch(exe, [a, b], [c]);
      ...
      // XOR
      exe = await backend.prepareKernel(
        Kernel.single(2, 4, AluExp.bitCombine(arg1, arg2, "xor")),
      );
      backend.dispatch(exe, [a, b], [c]);
      buf = (await backend.read(c)).buffer;
      expect(new Uint32Array(buf)).toEqual(
      ...
      exe = await backend.prepareKernel(
        Kernel.single(2, 4, AluExp.bitShift(sarg1, sarg2, "shl")),
      );
      ...
      exe = await backend.prepareKernel(
        Kernel.single(2, 4, AluExp.bitShift(rarg1, sarg2, "shr")),
      );
