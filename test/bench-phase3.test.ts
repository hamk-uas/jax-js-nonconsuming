...
describe("Phase 3 matmul benchmark", () => {
  ...
  test("512×512 benchmark", async ({ skip }) => {
    ...
    console.log(`  np.matmul:     ${t1.toFixed(3)} ms  (${gflops(t1)} GFLOP/s)`);
    console.log(`  tiled(16):     ${t2.toFixed(3)} ms  (${gflops(t2)} GFLOP/s)`);
    console.log(`  tt=[4,4]:      ${t3.toFixed(3)} ms  (${gflops(t3)} GFLOP/s)`);
    console.log(`  tt=[8,8]:      ${t4.toFixed(3)} ms  (${gflops(t4)} GFLOP/s)`);
    console.log(`  Speedup tt=[4,4] vs tiled(16): ${(t2 / t3).toFixed(1)}×`);
    console.log(`  Speedup tt=[8,8] vs tiled(16): ${(t2 / t4).toFixed(1)}×`);

    // Cleanup
    f_np.dispose(); f_base.dispose(); f_tt44.dispose(); f_tt88.dispose();
    a.dispose(); b.dispose();
    clearCaches();
    defaultDevice(prev);
  ...
  test("shader-dump: threadTile=[4,4] O5 check", async () => {
    ...
    if (navigator.gpu) {
      ...
      if (adapter) {
        ...
        console.log("  maxStorageBuffers:", adapter.limits.maxStorageBuffersPerShaderStage);
  ...
  test("kernel timing: batch dispatch", async ({ skip }) => {
    ...
    // If overhead is proportional, GFLOP/s stays constant.
    {
      ...
      for (const N of [64, 128, 256, 512, 1024]) {
        ...
        const fi = jit((A: np.Array, B: np.Array) => np.matmul(A, B) as np.Array);
        // warmup
        for (let j = 0; j < 3; j++) { const rr = fi(ai, bi); await blockUntilReady(rr); rr.dispose(); }
        const batch = 10;
        const rs: np.Array[] = [];
        ...
        fi.dispose(); clearCaches();
        const ms = (t1 - t0) / batch;
        const flops = 2 * N * N * N;
        const gf = flops / (ms * 1e-3) / 1e9;
        console.log(`  ${N}×${N}: ${ms.toFixed(1)} ms/call  (${gf.toFixed(2)} GFLOP/s)`);
      }
    }
    ...
    const configs = [
      { name: "np.matmul", fn: jit((A: np.Array, B: np.Array) => np.matmul(A, B) as np.Array) },
      { name: "tiled(16)", fn: jit((A: np.Array, B: np.Array) => lax.tiledMatmul(A, B, { Br: 16, Bc: 16, Bk: 16 })) },
      { name: "tt=[4,4]", fn: jit((A: np.Array, B: np.Array) => lax.tiledMatmul(A, B, { Br: 64, Bc: 64, Bk: 16, threadTile: [4, 4] })) },
      { name: "tt=[8,8]", fn: jit((A: np.Array, B: np.Array) => lax.tiledMatmul(A, B, { Br: 64, Bc: 64, Bk: 16, threadTile: [8, 8] })) },
    ];
    ...
    for (const { name, fn: f } of configs) {
      ...
      console.log(`  ${name}: ${msPerCall.toFixed(3)} ms/call  (${gflopsVal.toFixed(1)} GFLOP/s)`);
    }
