...
await deviceSuite((device) => {
  ...
  suite("jax.lax.linalg.qr()", () => {
    ...
    test("foriLoop grad with np.power (clearCaches regression)", () => {
      ...
      // because the jvpJaxprCache held consts from a disposed body jaxpr.
    ...
    test("JVP through QR matches finite differences", () => {
      ...
      const eps = device === "cpu" ? 3e-3 : 1e-4;
      using dAe = dA.mul(eps);
      using Ape = A.add(dAe);
      using R2 = f(Ape);
      using R2subR = R2.sub(R);
      using dR_fd = R2subR.div(eps);
      // The CPU Householder polyfill is float32-only, so a larger step avoids
      // forward-difference cancellation that does not affect the JVP itself.
