...
describe("customRoot", () => {
  ...
  test("gradient flows through closure parameters via IFT", () => {
    ...
    const loss = (b: np.Array) => {
      const f = (x: np.Array) => np.subtract(np.multiply(diag, x), b);
      const solve = (_f: (x: np.Array) => np.Array, _x0: np.Array) =>
        np.divide(b, diag);
      const tangentSolve = (_g: (x: np.Array) => np.Array, y: np.Array) =>
        np.divide(y, diag);

      using x = customRoot(f, np.zeros([3]), solve, tangentSolve);
  ...
  test("scalar root: solve a*x - b = 0", () => {
    ...
    const loss = (b: np.Array) => {
      const f = (x: np.Array) => np.subtract(np.multiply(5.0, x), b);
      const solve = (_f: (x: np.Array) => np.Array, _x0: np.Array) =>
        np.divide(b, np.array(5.0));
      const tangentSolve = (_g: (x: np.Array) => np.Array, y: np.Array) =>
        np.divide(y, np.array(5.0));

      using x = customRoot(f, np.array(0.0), solve, tangentSolve);
      return np.sum(x.mul(x));
    };
  ...
  test("composition with jit", () => {
    ...
    const loss = (b: np.Array) => {
      ...
      using x = customRoot(f, np.zeros([2]), solve, tangentSolve);
      return np.sum(x.mul(x));
    };
  ...
  test("vjp returns correct cotangents", () => {
    ...
    const rootFn = (b: np.Array) => {
      ...
      const tangentSolve = (_g: (x: np.Array) => np.Array, y: np.Array) =>
        np.divide(y, diag);
      return customRoot(f, np.zeros([2]), solve, tangentSolve);
    };
  ...
  test("non-linear root: square root via Newton's method", () => {
    ...
    const sqrtLoss = (a: np.Array) => {
      const f = (x: np.Array) => np.subtract(np.multiply(x, x), a);
      ...
      using x = customRoot(f, np.array(1.0), solve, tangentSolve);
      return np.sum(x);
    };
  ...
  test("gradient through multiple closure parameters", () => {
    ...
    const loss = (a: np.Array, b: np.Array) => {
      const f = (x: np.Array) => np.subtract(np.multiply(a, x), b);
      const solve = (_f: (x: np.Array) => np.Array, _x0: np.Array) =>
        np.divide(b, a);
      const tangentSolve = (_g: (x: np.Array) => np.Array, y: np.Array) =>
        np.divide(y, a);
