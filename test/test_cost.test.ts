import { evaluateTotalCost, CostFeatures } from "../src/tuner";
import type { BackendCapabilities } from "../src/backend";

const capsIntelOld: Partial<BackendCapabilities> = { dispatchOverheadUs: 25, bandwidthGBs: 30, tflops: 0.4, adapterVendor: "intel", adapterArchitecture: "gen_12lp", inferredVendorClass: "igp", maxComputeWorkgroupSizeX: 256 };
...
function testFixedEvaluate(features: CostFeatures, caps: Partial<BackendCapabilities>) {
...
function evalCaps(caps: Partial<BackendCapabilities>) {
  ...
  for (const c of baseCandidates) {
    ...
    console.log(`Br=${c.Br} Bc=${c.Bc} tt=${tt} -> cost=${cost.toExponential(4)}, threads=${numThreads}, dp=${features.depthPriv}, totR=${features.depthPriv*numThreads}`);
  }
}
